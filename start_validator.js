...
async function start() {
    ...
    if (accountList.length > 0) {
        ...
        const account = provider.signer.address;
    ...
    await throwIfNotContract(provider, config.tokenAddress, "Config variable tokenAddress")
    await throwIfNotContract(provider, config.contractAddress, "Config variable contractAddress")
    ...
    const validator = new Validator(accountList, address, provider, channel, config, fileStore, log, error)
